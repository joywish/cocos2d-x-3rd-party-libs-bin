...
public:
    ...
    static ChipmunkBody* bodyWithMass(cpFloat mass,cpFloatmoment);
    ...
    ///Guessing the moment of inertia is usually a bad idea. Use the moment estimation functions (cpMomentFor*()).
    ChipmunkBody* initWithMass(cpFloat mass,cpFloat moment);
    ...
    void applyForce(cpVect force,cpVect atLocalPoint);
    void applyForce(cpVect force,cpVect atWorldPoint);

    ///Apply an impulse to a rigid body.
    ///@param impulse An impulse in expressed in absolute (word) coordinates.
	///@param offset An offset expressed in world coordinates. Note that it is still an offset, meaning that it's position is relative, but the rotation is not.
    void applyImpulse(cpVect impulse,cpVect atLocalPoint);
    void applyImpulse(cpVect impulse,cpVect atWorldPoint);
    ...
    void updateVelocity(cpFloat dt,cpVect gravity,cpFloat damping);
